...
// If modifying these scopes, delete ../data_collector/token.json.
...
// The file ../data_collector/token.json stores the user's access and refresh tokens, and is
// created automatically when the authorization flow completes for the first
// time.
const TOKEN_PATH = "../data_collector/token.json";

// Load client secrets from a local file.
fs.readFile("../data_collector/credentials.json", (err, content) => {
  if (err) return console.log("Error loading client secret file:", err);
  // Authorize a client with credentials, then call the Google Classroom API.
  authorize(JSON.parse(content), deleteStudents);
});
 ...
 */
function deleteStudents(auth) {
  const classroom = google.classroom({ version: "v1", auth });
  classroom.courses.students.delete(
    {
        courseId:"340243970077",
        userId: "113460153041381506389",
    },
    (err, res) => {
