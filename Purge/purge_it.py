...
def main():
    ...
    # time.
    if os.path.exists('../data_collector/token.json'):
        creds = Credentials.from_authorized_user_file('../data_collector/token.json', SCOPES)
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                '../data_collector/credentials.json', SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open('../data_collector/token.json', 'w') as token:
